...
using namespace std;
int monopolyboard::turn=1;
monopolyboard* monopolyboard::head=nullptr;
...
monopolyboard::monopolyboard(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::monopolyboard)
{
    head=this;
    ui->setupUi(this);
...
void monopolyboard::on_pushButton_2_clicked()
{
...
void monopolyboard::sendGameMsg(){
    ...
    BufferPing.append(char(3));
    for(int i=0;i<2;i++){

        BufferPing.append(char(players[i]->getBank()));
        BufferPing.append(char(players[i]->getJailFree()));
        BufferPing.append(char(players[i]->getPosition()));
        BufferPing.append(char(players[i]->getJailStatus()));
        for(int ii=0; ii<40; ii++){
            BufferPing.append(char(players[i]->getOwned(i)));
        }
    }
    for(int i=0; i<40; i++){
        BufferPing.append(spaces[i]->getOwnership());
        BufferPing.append(spaces[i]->getRent());
...
void monopolyboard::receiveUpdates(char vals[], int size){
    for(int i=0; i<2; i++){
        head->players[i]->setBank(int(vals[i*44+0]));
        head->players[i]->setJailFree(int(vals[i*44+1]));
        head->players[i]->setPosition(int(vals[i*44+2]));
        head->players[i]->setJail(int(vals[i*44+3]));
        for(int ii=0; ii<40;ii++){
            if(int(vals[i*44+ii+4]))
                head->players[i]->setOwned(i);
        }
    }
    for(int i=0; i<40; i++){
        head->spaces[i]->setOwnership(int(vals[88+i*2]));
